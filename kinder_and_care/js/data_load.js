...
async function load() {
  ...
  */
  ...
  let response_kinders = await fetch('./data/노원구_유치원어린이집.json');
  kinders = await response_kinders.json();

  let response = await fetch('./data/노원구.json');
  let json = await response.json();
  
  function to_img_data(img) {
    let g = document.createElement('canvas').getContext('2d');
    g.canvas.width = img.width;
    g.canvas.height = img.height;
    g.drawImage(img, 0, 0);
    img_terrain_data = g.getImageData(0, 0, g.canvas.width, g.canvas.height);
  }
  
  let checkImage = path =>
    new Promise(resolve => {
        let img = new Image();
        img.onload = () => resolve(to_img_data(img));
        img.onerror = () => resolve({path, status: 'error'});
        img.src = path;
        img_terrain = img;
    });
  
  await checkImage('./img/terrain.png');
  ...
  let checkImage2 = (path, target) =>
    new Promise(resolve => {
        let img = new Image();
        img.onload = () => resolve(set_img(img));
        img.onerror = () => resolve({path, status: 'error'});
        img.src = path;
    });
  
  await checkImage2('./img/satmap.png', img_satmap);
  */
